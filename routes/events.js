...
router.post('/events2', async function (req, res) {
    try {
        const id = req.body.id;

        // Get all events for the given user from Redis
        const events = await redisClient.hGet(`userEvents`, id);

        // Parse events from JSON strings to objects
